...
스포티파이 기본 곡선은 위쪽이 가팔라서(아래 절반은 거의 안 움직임), `p`가 **1보다 작으면** 낮은 쪽이 올라와 슬라이더 전체가 쓸모 있어져요. `p = 1`은 리니어(보정 없음), `p`가 클수록 스포티파이 특유의 위쪽 쏠림에 가까워져요. 트레이나 패널의 **실시간 곡선 그래프**로 감으로 고르세요:

<div align="center"><img src="assets/curve.png" width="640" alt="스포티파이의 위쪽 쏠림을 펴서 슬라이더 전체를 쓸모 있게 만드는 거듭제곱 곡선"></div>

| 프리셋 | `p` | 느낌 |
|--------|----:|------|
| **완만** · *Gentle* | 0.3 | 가장 평탄 — 낮은 쪽 최대 보정 |
| **살짝 완만** · *Soft* | 0.5 | 부드러운 보정 |
| **리니어** · *Linear* | 1.0 | 리니어 — 추가 보정 없음 |
| **가파름** · *Steep* | 1.5 | 스포티파이 기본 곡선에 가깝게 |
| **스포티파이 기본** · *Spotify default* | 2.0 | 스포티파이 특유의 위쪽 쏠림 |

> 출발점일 뿐이니 취향껏 조절하세요. 설정되는 값이 스포티파이의 *진짜* 볼륨이라 내부는 아무것도 안 건드리고, 그 레벨이 모든 기기로 따라가요.

## 🚀 기능

- 🎚️ **조절 가능한 청감 곡선** — *완만 / Gentle (0.3)*부터 *스포티파이 기본 / Spotify default (2.0)*까지 5개 프리셋과 **실시간 곡선 그래프**.
